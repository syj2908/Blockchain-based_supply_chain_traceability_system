...
import "../contracts/LoanSystem.sol";

contract TestLoanSystem {
    LoanSystem loansystem = LoanSystem(DeployedAddresses.LoanSystem());
    string lenderID;
    string borrowerID;
    uint256 cashflowID;
    ...
    function testCheckCashFlow() public {
        uint256 initBalance;
        (initBalance, , ) = loansystem.checkCashFlow(cashflowID);
        Assert.equal(initBalance, cashNum, "Cashflow check failed.");
    }

    function testCreateTransaction() public {
        uint256 transactionID = loansystem.createTransaction(
            cashflowID,
            borrowerID,
            "111",
            829,
    ...
    function testDeleteLender() public {
        bool res = loansystem.deleteLender(lenderID);
        Assert.isTrue(res, "Delete Lender failed.");
    }

    function testDeleteBorrower() public {
        bool res = loansystem.deleteBorrower(borrowerID);
